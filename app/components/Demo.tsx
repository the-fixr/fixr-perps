...
// GMX on-chain MIN_COLLATERAL_USD = $1, but after fees (position fee + UI fee)
// the remaining collateral must still be >= $1. Setting to $2 for safety buffer.
    ...
    async function initFrameSDK() {
      try {
        ...
        if (mounted) {
          ...
          console.log('[Frame] context.client:', context?.client);
          console.log('[Frame] context.client.added:', context?.client?.added);
          console.log('[Frame] context.user:', context?.user);

          if (context && !context.client?.added) {
            try {
              console.log('[Frame] Prompting user to add mini app...');
              const result = await window.frame.sdk.actions.addMiniApp();
              console.log('[Frame] addMiniApp result:', JSON.stringify(result));

              // With Neynar managed webhook, notificationDetails may not be returned
              // The token is sent directly to our webhook URL instead
              console.log('[Frame] addMiniApp result.added:', result.added);
              console.log('[Frame] notificationDetails:', result.notificationDetails);

              if (result.added) {
                // Send welcome notification after delay for Neynar to store token
                const fid = context.user?.fid;
                console.log('[Frame] result.added is true, fid:', fid);
                if (fid) {
                  console.log('[Frame] Waiting 3s for Neynar to store token...');
                  setTimeout(async () => {
                    console.log('[Frame] Timer fired. Sending welcome notification to FID:', fid);
                    try {
                      const success = await sendWelcomeNotification(fid);
                      console.log('[Frame] Welcome notification result:', success);
                    } catch (notifErr) {
                      console.error('[Frame] Welcome notification error:', notifErr);
                    }
                  }, 3000);
                } else {
                  console.log('[Frame] No FID found in context, cannot send notification');
                }
              } else {
                console.log('[Frame] result.added is false or undefined');
              }
            } catch (addErr) {
          ...
          <div className="p-2 space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                <Tooltip text="Min $2 to ensure $1 remains after fees">
